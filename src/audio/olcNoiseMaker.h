	...
	static std::vector<std::wstring> Enumerate()
	{
		...
		for (int n = 0; n < nDeviceCount; n++)
			if (waveOutGetDevCaps(n, &woc, sizeof(WAVEOUTCAPS)) == S_OK) {
				std::string str = std::string(woc.szPname);
				std::wstring wstr = std::wstring(str.begin(), str.end());
				sDevices.push_back(wstr);
			}
		return sDevices;
	}
...
private:
	...
	{
		(reinterpret_cast<olcNoiseMaker*>(dwInstance))->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
