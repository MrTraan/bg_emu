...
void Basic_Gb_Apu::write_register(gb_time_t time, gb_addr_t addr, int data)
{
	apu.write_register(time, addr, data);
}

int Basic_Gb_Apu::read_register(gb_time_t time, gb_addr_t addr)
{
	return apu.read_register(time, addr);
}

void Basic_Gb_Apu::end_frame(gb_time_t length)
{
	time = 0;
